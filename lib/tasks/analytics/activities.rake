  ...
  task :course_constructions => [:environment, 'activities:duration'] do
    ...
    Course.where(workflow_state: 'available').find_each do |course|
      activity = course.teachers.map do |teacher|
        wiki_pages = WikiPage.where(workflow_state: 'active', context: course, created_at: $duration).count + WikiPage.where(workflow_state: 'active', context: course.context_modules, created_at: $duration).count
        attachments = Attachment.where(workflow_state: 'processed', context: course, created_at: $duration).count +
                      Attachment.where(workflow_state: 'processed', context: course.context_modules, created_at: $duration).count +
                      Attachment.where(workflow_state: 'processed', context: course.assignments, created_at: $duration).count +
                      Attachment.where(workflow_state: 'processed', context: Assignment.where(context: course.context_modules), created_at: $duration).count
        ...
        wiki_pages + context_modules + attachments + quizzes + assignments
      end.sum
